...
ideogram <- list(
  `$schema` = vega_schema("vega_lite"),
  data = list(values = as.data.frame(example_anno)),
  mark = "rect",
  encoding = list(
    x = list(field = "start", type = "quantitative", 
             scale = list(domain = c(min(start(example_anno)), max(end(example_anno))))),
    x2 = list(field = "end", type = "quantitative"),
    y = list(field = "gene_id", type = "ordinal")
  )  
)
