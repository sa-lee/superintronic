...
cvg_over_gene_plot <- function(cvg_features, gene_id) {
  cvg_features %>% 
    ...
    theme(axis.text.x = element_text(angle = 60, vjust = 60))
}
...
cvg <- lapply(cvg, function(x) x[seqnames(main_chr)])
...
features <- txdb %>%
  genes() %>% 
  mutate(
    n_overlaps_self = count_overlaps(., .),
    features_exons = txdb %>% 
      exonsBy("gene") %>% 
      reduce() %>% 
      unname(),
    features_introns = . %setdiff% 
      features_exons %>% 
      unname()
  )
...
features %>% 
  group_by(n_overlaps_self) %>% 
  summarise(n = n())

# no overlaps, no 1 exon only lists
one_hits <- features %>% 
  filter(n_overlaps_self == 1L, lengths(features_exons) > 1)

# compute views lookup in parallel
exon_scores <- lapply(
    seq_along(features),
    FUN = function(i) {
           make_views(seqnames(features[i]), features$features_exons[i])
         }  
    )
...
features_exons <- features_exons[lengths(features_exons) > 0]
...
Need to double check for multiple genes over a single score -> artificially
inflates the coverage. 
...
First, we create the scatter, which is used to perform filtering:
```{r vw-scatter}
