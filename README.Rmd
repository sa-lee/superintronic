...
Explore intron signal in high-throughput (RNA) sequencing data via coverage
estimation.
...
a set of BAM files. The parallel computation is achieved with 
`BiocParallel`. The result is a long GRanges containing columns called `score`
and `source`. An optional GRanges may be passed to this function to restrict
coverage to compute over selected contigs/chromosomes.
...
This requires an additional `data.frame` that contains a column that matches
the BAM file input. 
