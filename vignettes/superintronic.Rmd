...
per_replicate_avg <- per_replicate_avg %>% 
  merge(design[, c("Replicate", "Kit", "File")], 
        by.x = "source", 
        by.y = "File") 
...
* Calculate mean of e_g over all genes, E `gene_avg`
* Number of intron bases with coverage above E
* This gives the values E, L and U. 
...
save_plot <- function(.x, .y) {
  ...
  p <- anno_sub %>% 
    ...
    view_coverage_within_gene(cvg_over_features, ., hline, "Kit")
